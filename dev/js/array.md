...
|pop|удаляет последний элемент|возвращает удаленный элемент|да|
|map|||нет|
|filter|ищет все элементы, на которых функция-колбэк вернет true||нет|
|concat|склеивает массивы||нет|
|every||||
|find||||
|findIndex||||
|forEach|выполняет указанную функцию один раз для каждого элемента в массиве|ничего не возвращает|нет|
|from||||
|join|склеивает элементы массива в строку||нет|
|reduce|||нет|
|reverse|||да|
|shift|удаляет первый элемент|возвращает удаленный элемент|да|
|slice|чтобы из массива получить подмассив|возвращает новый подмассив|нет|
|some||||
|sort|||да|
|splice|||да|
|unshift|добавляет в начало|возвращает новую длину|да|
